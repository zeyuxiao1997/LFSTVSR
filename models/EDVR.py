...
class EDVRx4(nn.Module):
    def __init__(self, opt):
        ...
        if self.is_predeblur:
            ...
            self.conv_1x1 = nn.Conv2d(self.nf, self.nf, 1, 1, bias=True)
        else:
            if self.HR_in:
                self.conv_first_1 = nn.Conv2d(3, self.nf, 3, 1, 1, bias=True)
                self.conv_first_2 = nn.Conv2d(self.nf, self.nf, 3, 2, 1, bias=True)
                self.conv_first_3 = nn.Conv2d(self.nf, self.nf, 3, 2, 1, bias=True)
            else:
        ...
        if self.w_TSA:
            ...
            self.tsa_fusion = nn.Conv2d(self.nframes * self.nf, self.nf, 1, 1, bias=True)
...
class EDVRx8(nn.Module):
    def __init__(self, opt):
        #  nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        #          predeblur=False, HR_in=False, w_TSA=True
        super(EDVRx8, self).__init__()
        self.nf = 64
        self.nframes = 7
        self.center = self.nframes//2 
        self.groups = 8
